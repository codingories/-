<template>
  <div class="dashboard-container">
    <el-row>
      ...
      <el-dialog :visible.sync="dialogVisible" :before-close="handleClose" title="添加流程" width="30%">
        <div v-for="(item, i) in addItemList" :key="i">
          <el-checkbox v-model="item.checked">{{ item.flowName }}</el-checkbox>
        </div>
    ...
    <el-dialog :visible.sync="addrepairshow" :before-close="handleClose" title="增加报修" width="700px">
      <el-form ref="addrepairform" :model="addrepairform" :rules="rules" label-width="100px">
        <el-form-item label="报修地点" prop="location" class="setInline">
          <el-input v-model="addrepairform.location" placeholder="请填写报修地点"/>
        </el-form-item>
        <el-form-item label="报修教室" prop="classroom" class="setInline">
          <el-input v-model="addrepairform.classroom" placeholder="请填写报修教室"/>
        </el-form-item>
        <el-form-item label="标题" prop="title" class="setInline">
          <el-input v-model="addrepairform.title" placeholder="请填写标题"/>
        </el-form-item>
        <el-form-item label="问题描述" prop="repairdetail">
          <el-input v-model="addrepairform.repairdetail" placeholder="请填写问题描述" type="textarea"/>
        </el-form-item>
  ...
  methods: {
    ...
    getApplies() {
      getEntries({ access_token: this.access_token }).then(res => {
        ...
        const statusMap = { '0': '进行中', '9': '通过', '-1': '驳回' }
        for (const i of list) {
          const obj = {}
          let tempstr = ''
    ...
    getApprovals() {
      getUserFlowsApprovals({ access_token: this.access_token }).then(res => {
        const list = res.data.procs.slice(0, 3)
        const statusMap = {
          '0': '进行中',
          '9': '通过',
          '-1': '驳回',
    ...
    myApplyDetail() {
      ...
      const uri = 'http://' + host + '/#/emp'
      window.location = uri
    },
    myApprovalDetail() {
      const host = window.location.host
      const uri = 'http://' + host + '/#/flow'
      window.location = uri
    },
    ...
    formConfirmStatus() {
      const access_token = this.access_token
      const flow_id = this.flow_id
      const flow_name = this.flow_name
      const tpl = this.form
      const obj = { access_token, flow_id, flow_name, tpl }
      if (tpl['title'].trim()) {
